...
species_data = [line.split("\t")[3,4,6] for line in species_data]
    ...
    for name in species_data:
        ...
        if name[2] == match_name_coverage:
            ...
            with open(item) as infile:
                infile = [line.split("\t") for line in infile.readlines()]

                for line in infile:
                    if line[0] == "genome":
                        line[0] = f"genome_{species}_{subspecies}"

                        infile.write(f"{"\t".join(line)}\t{species}\t{subspecies}\n")
...
for name, df_grouped in df.groupby("gnm"):
    ...
    data["species"].append(str(set(df_grouped["Species"])))
    data["subspecies"].append(str(set(df_grouped["Subspecies"])))

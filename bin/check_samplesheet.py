...
def print_error(error,line):
    print(f"ERROR: Please check samplesheet -> {error}\nLine: '{line.strip()}'")
    sys.exit(1)
    ...
    while True:
        ...
        if line:
            ...
            if len(lspl) != len(header):
                print_error("Invalid number of columns (minimum = {len(header)})!",line)
    ...
    if len(sampleRunDict) > 0:
        ...
        for sample in sorted(sampleRunDict.keys()):
            ...
            if not all(x[:2] == sampleRunDict[sample][0][:2] for x in sampleRunDict[sample]):
                print_error("Multiple runs of a sample must be of the same datatype","Sample: {sample}")
